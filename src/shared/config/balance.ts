...
export const balance = {
  ...
  arcade: {
    ...
    games: {
      ...
      // Memory Matrix (redesigned 2026-06-29 — the old single-dot UFOV ring was too easy;
      // multi-agent research found the arcade lacked a PARALLEL grid-flash visual-memory game,
      // distinct from SpanRecall's serial/ordered Corsi). A SET of cells flashes at once, then
      // hides; tap them all back in any order to climb a level (+1 cell, shorter flash, the grid
      // periodically grows). Tone-compliant difficulty: a wrong tap flashes red + drops you a
      // level — never lives/game-over; the round is bounded by the clock. Per-tier knobs:
      // setStart = cells on the first level, grid = starting NxN, gridEvery = levels per grid
      // grow (capped at gridMax), exposureStart→exposureFloor shrink by exposureStep each level.
      flashrecall: { name: 'Flash Recall', icon: 'Eye', color: 'var(--skill-speed)', blurb: 'Memorize the lit cells at a glance, then tap them back — visual memory (matrix).', howToPlay: 'A set of cells flashes at once, then vanishes. Tap every cell that lit up (any order). Clear them all to climb a level — one more cell each time, a shorter flash. A wrong tap flashes red and drops you a level. Race the clock for your highest level.', brainBenefit: 'Visual memory — grabbing a whole pattern in one glance and holding it.', seconds: 60, tiers: { easy: { setStart: 3, grid: 3, gridEvery: 4, gridMax: 5, exposureStart: 900, exposureFloor: 500, exposureStep: 45 }, medium: { setStart: 3, grid: 3, gridEvery: 3, gridMax: 6, exposureStart: 650, exposureFloor: 320, exposureStep: 45 }, hard: { setStart: 4, grid: 4, gridEvery: 2, gridMax: 7, exposureStart: 380, exposureFloor: 190, exposureStep: 40 } } },
